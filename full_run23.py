...
for dataset_label, reproc_label in zip(dataset_label_list, reproc_label_list):
    ...
    for SCORE_TYPE in SCORE_TYPE_true_names_list:
        names.append("char/other-s/{:}m100s{:}c100r{:}KId0{:}".format(dataset_label, SIGMA, reproc_label, SCORE_TYPE))
    #names.append("char/MoG{:}m100s3c100r{:}KIbl0".format(dataset_label, reproc_label))
    contamination_ROC_compare(names,
                              methods=["MinD", "KNC5", "MLLED", "MLL1D", "MLLN"], 
...
for SCORE_TYPE in SCORE_TYPE_true_names_list:
    for i in range(len(dataset_list)):
        shutil.copy("char/other-s/{:}m100s{:}c100r{:}KId0{:}/".format(dataset_label_list[i], SIGMA, reproc_label_list[i], SCORE_TYPE)+"res.pickle", output_folder+"ROC_pickles/ROC_{:}_{:}".format(dataset_label_list[i], SCORE_TYPE))
...
shutil.copy("char/other-s/QCDh5+toph5m100s{:}c100rnoneKId0logLds/dimensions.png".format(SIGMA), output_folder+"dimensions.png")
