...
for labeling, method, iii in zip(labelings, methods, [0, 1]):
    ...
    for i, counts_windows in enumerate(counts_windows_boot):
        ...
        res_list.append(sliding_cluster_performance_evaluation(counts_windows, save_path=save_path, filterr="med", plotting=False, labeling=labeling, verbous=False))
        if i % 100==0:
            print(i)
    ...
    plt.plot([1], [1], alpha=0, label=r"$n=0.5,\sigma=1$, method {:}".format(method))
    ...
    for c, path, col in zip(contamiantions, cont_paths, colors):
        ...
        for jj in range(10):
            ...
            res=sliding_cluster_performance_evaluation(counts_windows=counts_windows, save_path=path, filterr="med", plotting=False, labeling=labeling, verbous=False)
            print(res["chisq_ndof"])
    ...
    plt.plot([1], [1], alpha=0, label=r"$n=0,\sigma=0$, method {:}".format(method))
    colors=["red", "orange", "gold"] 
    for c, path, col in zip(contamiantions, cont_paths, colors):
        arr=[]
        ps=[]
