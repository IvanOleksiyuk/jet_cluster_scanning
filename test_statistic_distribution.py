...
import pickle
from sliding_cluster_performance_evaluation import sliding_cluster_performance_evaluation
import numpy as np
import random
from matplotlib.ticker import MaxNLocator
random.seed(a=2, version=2)
plt.close("all")
save_path="char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID1/"
...
for i in range(46): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID0/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(40): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID1/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(37): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID2/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(44): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID3/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(18): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID4/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(4): 
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewsqrtsme1ID5/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
...
for i, counts_windows in enumerate(counts_windows_boot):
    ...
    res_list.append(sliding_cluster_performance_evaluation(counts_windows, save_path=save_path, filterr=filterr, plotting=False, labeling=labeling, verbous=False))
    if i % 100==0:
        print(i)
...
cont_paths=["char/0kmeans_scan/BS26w60wk50ret0con0.1W100ste200rewsqrtsme1ID10/",
...
plt.plot([1], [1], alpha=0, label=r"$n=0.5,\sigma=1$, method {:}".format(mehtod))
...
for c, path, col in zip(contamiantions, cont_paths, colors):
    ...
    for jj in range(10):
        ...
        res=sliding_cluster_performance_evaluation(counts_windows=counts_windows, save_path=path, filterr=filterr, plotting=False, labeling=labeling, verbous=False)
        print(res["chisq_ndof"])
        arr.append(res["chisq_ndof"])
        ps.append((np.sum(chisq_list>=res["chisq_ndof"])+np.sum(chisq_list>res["chisq_ndof"]))/2/len(chisq_list))
    
    if np.mean(ps)==0:
        label="$\epsilon$={:.4f}, $<p><${:.4f}".format(c, 1/len(chisq_list))
    else:
        label="$\epsilon$={:.4f}, $<p>=${:.4f}".format(c, np.mean(ps))
    plt.axvline(np.mean(arr), color=col, label=label)
    plt.axvspan(np.mean(arr)-np.std(arr), np.mean(arr)+np.std(arr), color=col, alpha=0.15)
plt.legend(loc=1)
plt.yscale("log")
#plt.xlim((0, 30))
...
plt.savefig("plots2/chi_s1sqrt_"+labeling+".png", bbox_inches="tight")
...
for i in range(47): #26
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewnonesme0ID0/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
for i in range(49): #26
    res=pickle.load(open("char/0kmeans_scan/BS26w60wk50MBret0con0.0W100ste200rewnonesme0ID1/"+"res{0:04d}.pickle".format(i), "rb"))
    counts_windows_boot+=res["counts_windows_boot"]
...
contamiantions=[0.01, 0.005, 0.0025]
cont_paths=["char/0kmeans_scan/BS26w60wk50ret0con0.1W100ste200rewnonesme0ID10/",
...
ax = plt.gca()
ax.set_xticks([0, 1, 2])
plt.savefig("plots2/chi_s0none_"+labeling+".png", bbox_inches="tight")
