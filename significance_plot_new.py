...
def make_last_valid_invalid(invalid):
    shape = invalid.shape
    invalid_inds = np.where(invalid)[0]
    if invalid_inds.size == 0:
        print("No invalid values found, ignoring this function call.")
        return invalid    
    invalid_inds = np.array(list(invalid_inds)+[invalid_inds[-1]+1, ])
    invalidd = np.full(shape, False)
    invalidd[invalid_inds] = True
    return invalidd
...
def significance_plot(plot_idealised = True, 
                      ...
                      style="band+lowstat",
                      use_cs_name=False,
                      plot_name = "plots/main/significances_responce.pdf",
                      label_list = None,
                      color_list = None,
                      CS_list = None):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for results, name in zip(results_list, name_list):
        ...
        if color_list is not None:
            ...
            color = "blue"
        draw_line_yerr(x_CS, median_CS, [err_low_CS, err_high_CS], label=label, style=style, color=color)
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        print("CS")
...
if __name__ == "__main__":
    CS_list =[]
    CS_list += [r"plots/ts_distribs/V4prep05_1_maxdev3_msdersCURTAINS_15mean_results.pickle"]
    CS_list += [r"plots/ts_distribs2/V4prep05_1_maxdev3_msdersCURTAINS_15mean_sig_reg_results.pickle"]
    CS_list += [r"plots/ts_distribs2/V4prep05_1_maxdev3_msdersCURTAINS_15mean_ignore_signal_results.pickle"]
    color_list = ["blue", "green", "red"]
    label_list = ["Cluster Scanning", "Cluster Scanning (signal region)", "Cluster Scanning (ignore signal)"]
    significance_plot(plot_idealised = False,
                      plot_realistic = True, 
                      plot_name="plots/main/significances_sig_reg_ignore.pdf",
                      use_cs_name= False,
                      CS_list = CS_list,
    ...
    CS_list =[]
    CS_list += [r"plots/ts_distribs/V4prep05_1_maxdev3_msdersCURTAINS_15mean_results.pickle"]
    CS_list += [r"plots/ts_distribs2/V4prep05_1_maxdev3+fit_msdersCURTAINS_15mean_results.pickle"]
    color_list = ["blue", "purple"]
    label_list = ["Cluster Scanning", "Cluster Scanning (fit)"]
    significance_plot(plot_idealised = False,
                      plot_realistic = True, 
                      plot_name="plots/main/significances_3par_fit.pdf",
                      use_cs_name= False,
                      CS_list = CS_list,
                      color_list = color_list,
                      label_list = label_list)