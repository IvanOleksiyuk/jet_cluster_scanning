...
def sliding_cluster_performance_evaluation(counts_windows=None,
    plotting=True,
    filterr="med",
...
#/home/ivan/mnt/cluster/k_means_anomaly_jet/char/0kmeans_scan/
    ...
    countnrm_windows_std=(countnrm_windows-countnrm_sum-baselinenrm)/countnrm_windows_sigma
    ...
    if filterr=="lowpass":
        ...
        for i in range(k):
            num_der_counts_windows[i]= butter_lowpass_filter(num_der_counts_windows[i], cutoff, fs, order)

    elif filterr=="med5":
        for i in range(k):
            num_der_counts_windows[i]= scipy.signal.medfilt(num_der_counts_windows[i], [5])
    
    elif filterr=="med":
        for i in range(k):
            num_der_counts_windows[i]= scipy.signal.medfilt(num_der_counts_windows[i], [7])
    
    elif filterr=="med11":
        for i in range(k):
            num_der_counts_windows[i]= scipy.signal.medfilt(num_der_counts_windows[i], [11])
    ...
    anomaly_poor_sigma=anomaly_poor_sigma*(np.sum(anomaly_rich)/np.sum(anomaly_poor))
    anomaly_poor=anomaly_poor*(np.sum(anomaly_rich)/np.sum(anomaly_poor))
    
    chisq=np.mean((anomaly_rich-anomaly_poor)**2/(anomaly_rich_sigma**2+anomaly_poor_sigma**2))
    ...
    if plotting:
        ...
        plt.savefig(save_path+"kmeans_ni_mjj_total_statAllowed.png", bbox_inches="tight")
        ...
        plt.savefig(save_path+"kmeans_ni_mjj_max_statAllowed.png", bbox_inches="tight")
        ...
        plt.savefig(save_path+"kmeans_ni_mjj_norm_statAllowed.png", bbox_inches="tight")
        ...
        for j in range(k):
            if labels[j]:
                ...
                plt.plot(window_centers, countnrm_windows_s[j], color="blue", label=lab2)
                lab2=None
        
        ccccc="lime"
        lwd=1.5
        plt.plot(window_centers, np.mean(countnrm_windows_s, axis=0), color=ccccc, label="mean and SD", linewidth=lwd)
        plt.plot(window_centers, np.mean(countnrm_windows_s, axis=0)-np.std(countnrm_windows, axis=0), color=ccccc, linewidth=lwd)
        plt.plot(window_centers, np.mean(countnrm_windows_s, axis=0)+np.std(countnrm_windows, axis=0), color=ccccc, linewidth=lwd)
        
        ccccc2="orange"
        plt.plot(window_centers, baselinenrm, color=ccccc2, label="outlier robust\n mean and SD", linewidth=lwd)
        plt.plot(window_centers, baselinenrm-std_ignore_outliers(countnrm_windows), color=ccccc2, linewidth=lwd)
        plt.plot(window_centers, baselinenrm+std_ignore_outliers(countnrm_windows), color=ccccc2, linewidth=lwd)
        plt.fill_between(window_centers, baselinenrm-std_ignore_outliers(countnrm_windows), baselinenrm+std_ignore_outliers(countnrm_windows), alpha=0.4, color="lime")
        plt.legend()
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(window_centers, countnrm_windows_std[j], color="red", label=lab1)
                lab1=None
            else:
                plt.plot(window_centers, countnrm_windows_std[j], color="blue", label=lab2)
                lab2=None
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(window_centers, countnrm_windows_std[j]*np.sqrt(countnrm_windows[j]), color="red")
            else:
                plt.plot(window_centers, countnrm_windows_std[j]*np.sqrt(countnrm_windows[j]), color="blue")
                
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        plt.ylabel("deviation in SD")
        plt.savefig(save_path+"eval/norm-toatal-sigmas-special.png", bbox_inches="tight") 
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(squeeze(window_centers, sqf), squeeze(countnrm_windows_std[j], sqf), color="red")
            else:
                plt.plot(squeeze(window_centers, sqf), squeeze(countnrm_windows_std[j], sqf), color="blue")
                
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        plt.ylabel("deviation in SD")
        plt.savefig(save_path+"eval/norm-toatal-sigmas-squeezed.png", bbox_inches="tight") 
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(np.mean(countnrm_windows_std[j]**2), np.random.normal(), "r.")
            else:
                plt.plot(np.mean(countnrm_windows_std[j]**2), np.random.normal(), "b.")
        plt.xlabel("npd")
        ...
        plt.savefig(save_path+"eval/norm-toatal-sigmas-points.png", bbox_inches="tight")
        ...
        h, xedges, yedges, image = plt.hist2d(xs, ys, bins=(200, 40), cmap="gist_heat_r")
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        plt.ylabel("deviation in SD")
        plt.colorbar()
        plt.sca(axs[1])
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(window_centers, countmax_windows[j]-countmax_sum, color="red")
            else:
                plt.plot(window_centers, countmax_windows[j]-countmax_sum, color="blue")
        
        plt.plot(window_centers, np.mean(countmax_windows-countmax_sum, axis=0), color="lime")
        ...
        plt.fill_between(window_centers, np.mean(countmax_windows-countmax_sum, axis=0)-np.std(countmax_windows, axis=0), np.mean(countmax_windows-countmax_sum, axis=0)+np.std(countmax_windows, axis=0), alpha=0.4, color="lime")
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(middles(window_centers), num_der_counts_windows[j], color="red", label=lab1)
                lab1=None
            else:
                plt.plot(middles(window_centers), num_der_counts_windows[j], color="blue", label=lab2)
                lab2=None
        ...
        for j in range(k):
            if labels[j]:
                plt.plot(middles(window_centers), num_der_counts_windows[j]-num_der_countmax_sum, color="red")
            else:
                plt.plot(middles(window_centers), num_der_counts_windows[j]-num_der_countmax_sum, color="blue")
        plt.xlabel("window centre $m_{jj}$ [GeV]")
        plt.ylabel(r"$Median filtered \Delta[N_i(m_{jj})/sum(N_i(m_{jj}))]$")
        plt.legend()
        plt.savefig(save_path+"eval/num_der_counts_windows_-bg.png", bbox_inches="tight")
        
        #TSNE
        X_embedded = TSNE().fit_transform(num_der_counts_windows)
        plt.figure()
        plt.grid()
        plt.plot(X_embedded[:, 0][labels==1], X_embedded[:, 1][labels==1], ".", color="red")
        plt.plot(X_embedded[:, 0][labels==0], X_embedded[:, 1][labels==0], ".", color="blue")
        ...
        plt.grid()
        sigmas=2
        plt.fill_between(window_centers, anomaly_rich-anomaly_rich_sigma*sigmas, anomaly_rich+anomaly_rich_sigma*sigmas, alpha=0.2, color="red")
        plt.fill_between(window_centers, anomaly_poor-anomaly_poor_sigma*sigmas, anomaly_poor+anomaly_poor_sigma*sigmas, alpha=0.2, color="blue")
        
        plt.plot(window_centers, anomaly_poor, label=r"$\tilde{N}_l$normalised sum of anomaly poor clusters", color="blue")
        ...
        plt.plot(window_centers, anomaly_rich, label=r"$\tilde{N}_l$ sum of anomaly rich clusters $\tilde{\chi}^2/n_{dof}=$"+"{:.3f}".format(res["chisq_ndof"]), color="red")
        ...
        def f(x, w, n, mu, sig):
            return w*bg(x)+n*W*tf/np.sqrt(2*np.pi)/sig*np.exp(-(x-mu)**2/2/sig**2)
        
        p0=(1, 0, p0_mu, 20)
        print("p0_mu", p0_mu)
        rrr=scipy.optimize.curve_fit(f, window_centers, anomaly_rich, sigma=np.sqrt(anomaly_rich_sigma**2+anomaly_poor_sigma**2), p0=p0,
                                     bounds=([0, 0, lb, 10], [2, 10000, rb, (rb-lb)/2]))
        print(rrr[0])
        #likelyhood spectrum
        #plt.plot(window_centers, f(window_centers, *p0), color="green")
        chisq_fit=np.mean((anomaly_rich-f(window_centers, *rrr[0]))**2/(anomaly_rich_sigma**2+anomaly_poor_sigma**2))
        plt.plot(window_centers, f(window_centers, *rrr[0]), color="green", label="Curvefit $w=${:.03f}, $n=${:.01f}, \n $\mu=${:.01f}, $\sigma=${:.01f}, \n".format(*rrr[0])+r"$\tilde{\chi}^2/n_{dof}=$"+"{:.3f}".format(chisq_fit))
        plt.legend()
