...
metric_name="eS@ieB100"
plot_min_pop=False 
ylims=None
os.makedirs("plots/param_scanBM/"+metric_name, exist_ok=True)
    ...
    for prep in list_prepr_names:
        ...
        for scr in list_score_names:       
            for i in range(len(list_k_clusters)):
                k=list_k_clusters[i]
                with open("char/MBORnDbg+ORnDsg2000_+ORnDsgm{:}s{:}c100r{:}{:}KId0{:}/res.pickle".format(k, sigma, prep, postpr, scr), "rb") as f:
                    res=pickle.load(f)
        ...
        for scr in list_score_names:       
            for i in range(len(list_k_clusters)):
                ...
                with open("char/MBORnDbg+ORnDsg2000_+ORnDsgm{:}s{:}c100r{:}{:}KId0{:}/res.pickle".format(k, sigma, prep, postpr, scr), "rb") as f:
                    res=pickle.load(f)
                metric[i]=get_metric_from_res(res, metric_name, "_mean")
        
            plt.plot(list_k_clusters, metric, "+-", label=scr+" mean score")
        ...
        for scr in list_score_names:       
            for i in range(len(list_k_clusters)):
                ...
                with open("char/MBORnDbg+ORnDsg2000_+ORnDsgm{:}s{:}c100r{:}{:}KId0{:}/res.pickle".format(k, sigma, prep, postpr, scr), "rb") as f:
                    res=pickle.load(f)
                metric[i]=get_metric_from_res(res, metric_name, "_max")
                min_pop[i]=np.min(res["cluster_counts_tr"])
            plt.plot(list_k_clusters, metric, "x--", label=scr+" maax score")
