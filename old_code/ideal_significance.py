...
save_path = "char/0kmeans_scan/ideal{:}k{:}{:}con{:}W{:}ste{:}ID{:}/".format(
    window, k, MiniBatch, signal_fraction, W, steps, ID
)
print(save_path)
HP = {
    "k": k,
...
allowed = np.concatenate(
    (np.zeros(len(mjj_sg) - num_true, dtype=bool), np.ones(num_true, dtype=bool))
)
