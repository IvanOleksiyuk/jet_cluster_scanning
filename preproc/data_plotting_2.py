...
print("plots will be saved in "+cfg.get("plots_directory"))
...
def plot_invariant_mass_spectrum(mjj_bkg, mjj_sig):
	...
	plt.grid()
	plt.legend()
	plt.savefig(cfg.get("plots_directory")+"data/mjj.pdf", bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(cfg.get("plots_directory")+"data/reprocessing.pdf", bbox_inches='tight')
...
#plt.savefig(cfg.get("plots_directory")+"data/jet_masses.pdf", bbox_inches='tight')
#plt.show()
...
def plot_averages(images_bkg, images_sig, cfg):
	...
	# Show the plot
	plt.savefig(cfg.get("plots_directory")+"data/avarage_images.pdf", bbox_inches='tight')
...
def plot_and_nonzero_pixels(images_bkg, images_sig, cfg):
	...
	plt.xticks([0, 20, 40, 60, 80, 100], ["0", "20", "40", "60", "80", "$\geq$100"])
	plt.legend()
	plt.savefig(cfg.get("plots_directory")+"data/non_zero_pixels.pdf", bbox_inches='tight')

def plot_and_save_background_jet_images(images_bkg, cfg, num_rows=2, num_cols=3):
    print("Plotting 4 background jet images")
    ...
    plt.tight_layout()
    output_path = cfg.get("plots_directory") + "data/background_jet_images.pdf"
